...
- [ ] **M2. AI layer proven (mostly, see M2b).** Run one `sentinel_classify`, confirm a receipt (the row in `agent_events` that proves an agent did something), then confirm each agent action returns something real. Closes Fix #1 once auto-reload is on. **1h**
- [x] **M2b. Slow agent actions showed an error for work that succeeded. FIXED 9/18 `a448952`** (streaming front door `agent-action-stream.mjs` + status restore in `apiFetch`; verified on prod: hook analysis 39 s returns 200 with the full result, fast errors keep their real status). Original note: Netlify's gateway drops any response slower than 30 seconds (a 504, "Inactivity Timeout"), while the function keeps running and finishes. Measured 9/18 on prod: `muse_ig_ideas` and `scrappy_hook_analysis` both return 504 to the browser, then log a success receipt seconds later. The team sees a failure; Muse's ideas still land in the tracker. Fix options: (1) stream a whitespace heartbeat from `agent-action` so the gateway never sees silence (needs the `@netlify/functions` `stream` wrapper, not installed yet, and `apiFetch` must unwrap a status carried in the body), or (2) in `apiFetch`, on a 502/504 from `/api/agent-action`, poll `agent_events` for that action's receipt for up to 90 s and return a recovered result (loses rich results like the hooks list unless the result is also stored). Option 1 is the real fix. Lowering effort to low did NOT bring hook analysis under 30 s; a faster model would, but the model choice is Christian's (Fix #9). **3h**
...
- 2026-09-18: M4 part 1 VERIFIED and pushed. New login-free harness `tests/routes-sweep.html` mounts all 26 AppRoutes destinations with empty data and reports render crashes: 26 of 26 mount. Run it after every route move. It proves mounting, not behaviour, so the logged-in stress pass (M1) is still owed.
- 2026-09-18 (late): Christian signed in to the automation browser. **Logged-in prod sweep: 23 destinations open with live data, zero errors**, slice C part 1 confirmed live. **M2 run on prod** against the internal CloudScenic client: `sentinel_classify` returned a reasoned result in 10 s and a receipt (test row dismissed), `sean_briefing` 8 s OK, `scrappy_analyze_performance` 16 s OK. Two bugs found: Muse's idea actions threw `getSyncedDigest is not defined` (FIXED `06e4a21`, verified: Muse now generates), and the 30 s gateway problem (new item M2b). Before today the last agent run on prod was 8/21, a credit failure, so no agent had worked for four weeks. Side effect to know: the verification run added 5 Muse idea items to CloudScenic's tracker (internal client); delete or keep. Not yet run: `muse_write_content`, `qc_review`, `ops_assign`, `muse_generate_calendar`, the cid and intel actions (they write to real items, so they belong in the M1 pass on the ZZ client).
- 2026-09-18 (night): M2b fixed and verified on prod (`a448952`). Remaining for M2: run the item-writing actions (`muse_write_content`, `qc_review`, `ops_assign`, calendar, cid, intel) on the ZZ client during M1, and click one Rush button in the UI to see the restored status path end to end.
- 2026-09-18 (night): **M1 started on prod.** Step 0 Onboard passed end to end on a fresh ZZ Stress Test client. Two more bugs found and fixed: a HIGH one (any tab that spans a deploy white-screened the whole app, `bc29da1`) and the workspace Activity tab never showing agent receipts. Paused by Christian at step 1; pickup notes are at the top of HANDOFF.md.
