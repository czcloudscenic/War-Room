...
-- 2. Add client_id to existing data tables (nullable for now — backfill below,
...
-- 3. Backfill: tag existing rows as VitalLyfe's data if the dev seed exists.
do $$
declare
  vl_id uuid;
begin
  ...
  if vl_id is null then
    raise notice 'VitalLyfe seed row missing; skipping client_id backfill';
  else
...
-- 4. Indexes for scoped queries (every list endpoint will filter by client_id + sort by ts)
